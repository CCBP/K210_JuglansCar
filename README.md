...
|--- PCB
|      |---3D drawing 3D模型文件
|      |---JuglansPCB 用于将K210与ESP32连接到小车驱动扩展板的PCB工程目录，修改后的最新版本
|      |---JuglansPCB_old 目前实际使用的PCB，但存在串口选择错误以及电源处未加电容这两个问题
...
# 已知问题
- 🔴在用户控制模式(`user`)下，`__receive_callback()`会出现`__iadd__`错误，但只捕获到一次该错误（已尝试修复，但不确定是否成功解决）；
- 🔴在采集模式(`rcording`)下，有时会出现`MemoryError`错误（已定位错误出现在`dataset.py`中的第87行保存图像，但未找到好的解决办法）；
- 🔴当开始遥控后再通过按键切换运行模式会导致`core dump: misaligned load`错误，推测原因为指针异常；
